...
(function() {
  ...
  */
  ...
  var EventDispatcher, EventUtils, Main, get, has3d, init, set,
  ...
  */
  ...
  Main = (function(_super) {
    ...
    Main.prototype.processingInstance = null;
    ...
    Main.prototype.delay = .3;

    Main.prototype.items = [
      {
        type: 0,
        rad: 100,
        x: 300,
    ...
    Main.prototype.testVertices = null;

    Main.prototype.showDEBUG = false;

    function Main() {
      this.area = __bind(this.area, this);
      this.baryCentricTestByArea = __bind(this.baryCentricTestByArea, this);
      this.initProcessing = __bind(this.initProcessing, this);
      this.clickBtnADD = __bind(this.clickBtnADD, this);
      this.keyBoadEvent = __bind(this.keyBoadEvent, this);
      this.sketch = new Processing.Sketch();
      this.canvas = $('.sketch')[0];
      this.sketch.attachFunction = this.initProcessing;
      this.processingInstance = new Processing(this.canvas, this.sketch);
      $("input[name='btn1']").on('click', this.clickBtnADD);
      $(document).keypress(this.keyBoadEvent);
      $(document).keyup(this.keyBoadEvent);
    }

    Main.prototype.keyBoadEvent = function(e) {
      this.showDEBUG = e.type === "keypress" && e.keyCode === 32;
      return false;
    };

    Main.prototype.clickBtnADD = function(e) {
      var data, hit, i, item, newTriangle, xPos, yPos, _i, _ref;
      item = this.processingInstance.loadShape("svg/butterfly_bottom.svg");
      hit = item.children[item.children.length - 1].vertices;
      newTriangle = [];
      xPos = Math.random() * window.innerWidth;
      yPos = Math.random() * window.innerHeight;
      for (i = _i = 1, _ref = hit.length; _i < _ref; i = _i += 1) {
    ...
    Main.prototype.initProcessing = function(processing) {
      var _this = this;
      ...
      processing.mouseMoved = function() {
        var i, item, j, nX, nY, _i, _ref;
        nX = _this.processingInstance.mouseX;
        nY = _this.processingInstance.mouseY;
        i = _this.items.length;
        while (i-- > 0) {
          item = _this.items[i];
          if (item.hit) {
            for (j = _i = 0, _ref = item.hit.length; 0 <= _ref ? _i < _ref : _i > _ref; j = 0 <= _ref ? ++_i : --_i) {
              if (_this.baryCentricTestByArea(item.hit[j].x1, item.hit[j].y1, item.hit[j].x2, item.hit[j].y2, item.hit[j].x3, item.hit[j].y3, nX, nY)) {
    ...
    Main.prototype.baryCentricTestByArea = function(x1, y1, x2, y2, x3, y3, px, py) {
      var a, a1, a2, a3;
      a = this.area(x1, y1, x2, y2, x3, y3);
      if (a === 0) {
        return false;
      }
      a1 = this.area(x2, y2, x3, y3, px, py) / a;
      if (a1 < 0) {
        return false;
      }
      a2 = this.area(x3, y3, x1, y1, px, py) / a;
      if (a2 < 0) {
        return false;
      }
      a3 = this.area(x1, y1, x2, y2, px, py) / a;
      if (a3 < 0) {
        return false;
      }
      return true;
    ...
    Main.prototype.area = function(x1, y1, x2, y2, x3, y3) {
      var v1x, v1y, v2x, v2y;
      v1x = x1 - x3;
      v1y = y1 - y3;
      v2x = x2 - x3;
