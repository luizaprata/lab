...
class Main extends EventDispatcher
	...
	init:()=>
		...
		ballTexture = THREE.ImageUtils.loadTexture('assets/item.png')
		ballMaterial = new THREE.SpriteMaterial({ map: ballTexture })
		# ballMaterial = new THREE.SpriteMaterial({ color: 0xff0000 })

		for i in [0...1000]
			particle = new THREE.Sprite(ballMaterial)
